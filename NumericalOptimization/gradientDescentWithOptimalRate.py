...
# In[8]:
"""
   find the optimal by find nextLevel's local minimum
"""
def findOutLearnRate(point, direction):
    x, y, z = symbols('x y z')
    f = funcformat()
    x1 = point[0] + z * direction[0]
    y1 = point[1] + z * direction[1]
    f = f.subs([('x', x1), ('y', y1)])
    f = f.diff(z)
    return solve(f, z)[0]
...
def animate(i):
    ...
    searchDirection = findout_NegderivateF(diffEquation, StartPoint)
    # find the learning rate of lambda
    learningRate = findOutLearnRate(StartPoint, searchDirection)
    descent_x.append(StartPoint[0])
    descent_y.append(StartPoint[1])
    for i in range(0, 2):
        StartPoint[i] = StartPoint[i] + learningRate * searchDirection[i]
    plt.clf()
    plt.contour(x, y, funcValue(x, y), levels=10)
